...
> **중요**: 스킬 실행 중 모든 응답은 아래 형식을 따릅니다. 각 단계가 시작/완료될 때마다 진행 현황 보드를 갱신해 출력합니다.

### 1. 현재 작업 표시

각 응답 시작 시 **현재 수행 중인 업무**를 한 줄로 명시:
...
주요 단계 전환 시점(단계 시작 / 단계 완료 / 항목 그룹 완료)마다 아래 보드를 출력:
...
- `[ ]` — **미완료** (대기 중)
- `[x]` — **완료**
- `[~]` — **점검 제외** (예: 항목 5는 정적 분석에서 제외, runtime 인자 없을 때 항목 17~20)
- `[!]` — **취약/주의 발견** (완료되었으나 취약점이 확인된 항목)

### 4. 출력 시점

- **단계 시작 시**: 현재 작업 한 줄 + 진행 현황 보드
- **단계 완료 시**: 완료 항목 체크 후 진행 현황 보드 갱신
- **단계 중간**: 현재 작업 한 줄만 갱신 (보드 생략 가능)
- **최종 완료 시**: 모든 항목 체크된 보드 + 보고서 경로 안내

### 5. 예시

```
🔄 현재 작업: 2단계 정적 분석 — Manifest 기반 분석(항목 2, 7, 8, 9) 완료
...
다음 시점에서는 반드시 **진행 현황 보드 전체**를 출력합니다. 생략 금지.

| 체크포인트 | 시점 |
|---|---|
| CP-0 | 0단계 시작 직전 |
| CP-0.5 | 0단계 완료 직후 (디컴파일/IPA 추출 성공 확인 후) |
| CP-1 | 1단계 완료 직후 (앱 기본 정보 수집 완료) |
| CP-2a | 2단계 Manifest 기반 분석(4개 항목) 완료 시 |
| CP-2b | 2단계 파일 구조 분석 완료 시 |
| CP-2c | 2단계 smali/strings 분석(나머지 항목) 완료 시 |
| CP-3 | 3단계 보고서 작성 완료 시 |
| CP-rt | (runtime 인자 시) 2.5단계 각 항목 완료 시 |

**추가 규칙**:
- 보드에는 항상 진행률 카운터(`N/16` 또는 `N/20`)를 포함합니다.
- 각 Grep/Bash 호출 **직전**에 `🔄 현재 작업: {단계명} — {항목 번호·명}` 한 줄을 출력합니다. 병렬 호출 시에도 한 줄로 묶어 표기합니다.

### 7. 3단계 완료 직후 자체 검증 블록 (강제)

보고서 Write 직후 아래 블록을 반드시 출력합니다.

```
✅ 자체 검증
- 커버리지: 16개(또는 20개) 항목 모두 판정 기재 여부
- 근거: 각 판정이 Grep/파일 출력 또는 사용자 응답에 근거하는지
- 포맷: 보고서 템플릿과 일치 여부
- 엣지: 놓친 패턴 재탐색 결과 (없으면 '없음')
```

검증에서 누락·근거 부족이 드러나면 보고서를 수정한 뒤 다시 이 블록을 출력합니다.
